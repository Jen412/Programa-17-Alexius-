...
public class Prueba{
   public static void main(String args[]) {
      ...
      do
      {
         System.out.print("\nIngrese el numero de simbolos que contiene el alfabeto: ");
   ...
   public static char[] alfabeto(int numAlf){
      ...
      boolean ban = true;
      int cont=0;
      System.out.println("\nINGRESE LOS SIMBOLOS DEL ALFABETO");
      for (int x=0;x<numAlf;x++){
         do
         {
            System.out.print("Ingrese el simbolo "+(x+1)+" del alfabeto: ");
            auxCar = leer.nextLine();
            //no debe aceptar el null o enter
            if (auxCar.length()!= 0)
            {
               if (auxCar.length()==1)
               {  
                  ...
                  if(cont==0){
                     ...
                     cont=0;
                     System.out.println("Simbolo repetido");
                     ban = false;
                  }        
               } 
               else
               {
                  System.out.println("Solo debe ingresar un simbolo");
                  ban = false;
               }
            }
            else
            {
               System.out.println("Debe ingresar un simbolo");
               ban = false;
            }   
         }while (ban = true);
      }
